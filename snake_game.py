...
class Snake(object):
    ...
    # 뱀 이동
    def move(self):
        overSound = pygame.mixer.Sound( "assets/boo.mp3" )
    ...
    # 뱀이 먹이를 먹을 때 호출
    def eat(self):
        eatingSound = pygame.mixer.Sound( "assets\Metallic_Clank.mp3" )
        eatingSound.play()
        self.length += 1
...
class Obstacle(object):
    ...
    def create(self):
        ...
        for i in range(5):
            x = random.randint(0, GRID_WIDTH - 1)
            y = random.randint(0, GRID_HEIGHT - 1)
            pos.append((x * GRID_SIZE, y * GRID_SIZE))
        self.position = pos

    # 장애물 그리기
    def draw(self, screen):
        for pos in self.position:
            pygame.draw.polygon(screen, RED, [[pos[0], pos[1]+GRID_SIZE], [pos[0]+GRID_SIZE, pos[1]+GRID_SIZE], [pos[0]+GRID_SIZE/2, pos[1]]])
